...
3. **Cleaning** — median or mean imputation (configurable) for numeric columns, mode imputation for categorical, automatic `log1p` transform for right-skewed columns, optional IQR outlier capping, dropped rows with a missing target.
4. **Feature engineering** — one-hot encoding for low-cardinality categoricals, frequency encoding for high-cardinality ones, full calendar + cyclical (sin/cos) expansion for datetime columns, KMeans spatial clustering + centroid distance for lat/lon pairs (plotted at an equal aspect ratio, colored by both target and cluster), and TF-IDF vectorization for free-text columns (instead of dropping them).
5. **Feature selection** — near-zero-variance columns are dropped automatically, and (when there are more than 50 engineered features — common once TF-IDF is involved) `SelectKBest` keeps only the top-scoring ones, fit on the training split only. A 2D PCA projection of the final feature set, colored by target/class, gives a quick "does this separate at all" read before modeling commits to anything.
...
7. **Modeling** — trains and compares up to **5 regression models** (Linear, Ridge, Lasso, Random Forest, Gradient Boosting) or **3 classification models** (Logistic Regression, Random Forest, Gradient Boosting) on an identical split (or, optionally, k-fold cross-validation) and automatically picks the best by held-out score. Test split size, which candidate models to include, each model's key hyperparameter (Ridge/Lasso's alpha range, Logistic Regression's `C`, Random Forest's depth/tree count, Gradient Boosting's learning rate/rounds), and an optional small grid search over that hyperparameter are all configurable.
...
The page is a 5-section stepper (Overview / How it works / Example Sheets / Live demo / Why it's real) navigated by Next/Prev buttons or the dot indicator rather than one long scroll. The three worked datasets (California Housing, Titanic, Retail Sales) live in their own **Example Sheets** section with a one-line result summary and a link to the full write-up — clicking a card loads it straight into the Live demo section and runs it. The Live demo section itself is upload-only, with a "Configure the pipeline" panel grouped by the same stages a data scientist would work through by hand — Data & cleaning, Feature engineering, Class balance, Models & hyperparameters (checking a model reveals its own tuning knobs), Evaluation strategy — and lets you tweak settings and re-run against the same file, or start over with a different one.
...
docs/methodology.md       step-by-step explanation of every automated decision, plus a
                          deliberate "known gaps" section (chi-square/ANOVA, standalone
                          clustering, true ARIMA, intervention analysis — not yet automated)
...
54 tests cover schema inference for every column role (numeric, categorical, datetime, geospatial, text, identifier, constant), cleaning (including multi-encoding CSV loading and outlier capping), feature engineering (including TF-IDF text vectorization), profiling (including redundant-feature-pair detection), both baseline+comparison modeling paths (tree-ensemble illustrative-tree, class imbalance, feature selection, cross-validation, configurable hyperparameters, and grid search), time-series diagnostics, and full end-to-end pipeline runs including edge cases (all-numeric data, EDA-only with no target, a geospatial dataset, a datetime+time-series dataset).
