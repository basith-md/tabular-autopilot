...
2. **Profiling** — missingness, skewness, IQR-based outlier counts, top category values, duplicate rows, and every numeric-column pair with |r| ≥ 0.9 flagged as redundant, independent of which model ends up winning.
3. **Segments & association tests** — standalone KMeans clustering over every numeric column (cluster count chosen by silhouette score, runs even without a target), plus, for classification targets, formal chi-square (categorical features), ANOVA (numeric, >2 classes), and Mann-Whitney U (numeric, 2 classes) tests against the target.
4. **Cleaning** — median or mean imputation (configurable) for numeric columns, mode imputation for categorical, automatic `log1p` transform for right-skewed columns, optional IQR outlier capping, dropped rows with a missing target.
5. **Feature engineering** — one-hot encoding for low-cardinality categoricals, frequency or out-of-fold target encoding (your choice) for high-cardinality ones, full calendar + cyclical (sin/cos) expansion for datetime columns, KMeans spatial clustering + centroid distance for lat/lon pairs (plotted at an equal aspect ratio, colored by both target and cluster), and TF-IDF vectorization for free-text columns (instead of dropping them).
6. **Feature selection** — near-zero-variance columns are dropped automatically, and (when there are more than 50 engineered features — common once TF-IDF is involved) `SelectKBest` keeps only the top-scoring ones, fit on the training split only. A 2D PCA projection of the final feature set, colored by target/class, gives a quick "does this separate at all" read before modeling commits to anything.
7. **Class imbalance handling** *(classification)* — target imbalance is detected and reported automatically; when enabled, balanced class weighting is applied (`class_weight="balanced"` where supported, `sample_weight` otherwise), and a balanced-accuracy metric is always reported alongside accuracy/F1/ROC-AUC.
8. **Modeling** — trains and compares up to **5 regression models** (Linear, Ridge, Lasso, Random Forest, Gradient Boosting) or **3 classification models** (Logistic Regression, Random Forest, Gradient Boosting) on an identical split (or, optionally, k-fold cross-validation) and automatically picks the best by held-out score, with a Wilcoxon signed-rank test (under cross-validation) checking whether the winner is actually significantly better than the runner-up. Test split size, which candidate models to include, each model's key hyperparameter (Ridge/Lasso's alpha range, Logistic Regression's `C`, Random Forest's depth/tree count, Gradient Boosting's learning rate/rounds), and an optional hyperparameter search (a small grid, or a broader randomized search over more parameters per model) are all configurable.
9. **Statistical diagnostics** — an interpretable OLS/Logit baseline alongside the model comparison: VIF-based multicollinearity pruning, coefficient significance, a Breusch-Pagan heteroscedasticity test, a Shapiro-Wilk normality test, residual and Q-Q plots. When a **tree ensemble** (Random Forest / Gradient Boosting) wins the comparison, a separate shallow decision tree is fit and drawn purely to illustrate the kind of split logic that ensemble is built from — the production model itself can't be visualized directly.
10. **Time-series diagnostics** *(auto-triggered)* — whenever a datetime column and numeric target are both present: trend strength, an Augmented Dickey-Fuller stationarity test, ACF/PACF, a formally order-selected ARIMA/SARIMA forecast (falling back to Exponential Smoothing if nothing converges), and — given an optional intervention date — an interrupted-time-series analysis of the level/slope shift around it.
11. **Reporting** — a single self-contained HTML report (`tabular_autopilot.report`), or the same analysis live in a Streamlit app with a tab per section.
...
The page is a 5-section stepper (Overview / How it works / Example Sheets / Live demo / Why it's real) navigated by Next/Prev buttons or the dot indicator rather than one long scroll. The three worked datasets (California Housing, Titanic, Retail Sales) live in their own **Example Sheets** section with a one-line result summary and a link to the full write-up — clicking a card loads it straight into the Live demo section and runs it. The Live demo section itself is upload-only, with a "Configure the pipeline" panel grouped by the same stages a data scientist would work through by hand — Data & cleaning, Feature engineering (including the frequency-vs-target-encoding choice), Class balance, Models & hyperparameters (checking a model reveals its own tuning knobs, plus an optional broad randomized search), Evaluation strategy, and an optional Time series section for testing an intervention date — and lets you tweak settings and re-run against the same file, or start over with a different one. Clustering and the formal association tests need no settings at all — they run automatically and show up in the report.
...
docs/methodology.md       step-by-step explanation of every automated decision, plus a
                          deliberate "known gaps" section (feature-vs-feature association
                          tests, full auto-ARIMA order selection, seasonal decomposition,
                          exhaustive hyperparameter tuning — not yet automated)
...
90 tests cover schema inference for every column role (numeric, categorical, datetime, geospatial, text, identifier, constant), cleaning (including multi-encoding CSV loading and outlier capping), feature engineering (including TF-IDF text vectorization and target encoding), profiling (including redundant-feature-pair detection), standalone clustering, formal hypothesis/association tests, both baseline+comparison modeling paths (tree-ensemble illustrative-tree, class imbalance, feature selection, cross-validation, configurable hyperparameters, narrow/broad search, and the Wilcoxon significance check), time-series diagnostics (ARIMA/SARIMA order selection and intervention analysis), and full end-to-end pipeline runs including edge cases (all-numeric data, EDA-only with no target, a geospatial dataset, a datetime+time-series dataset).
