...
The page is a 5-section stepper (Overview / How it works / Examples / Live demo / Why it's real) navigated by Next/Prev buttons or the dot indicator rather than one long scroll. The three worked datasets (California Housing, Titanic, Retail Sales) live in their own **Examples** section with a one-line result summary and a link to the full write-up — clicking a card loads it straight into the Live demo section and runs it. The Live demo section itself is upload-only, with an "Adjust settings" panel — test split size, in-browser row cap, numeric imputation strategy, which candidate models to include, and the modeling-rigor toggles below — and lets you tweak settings and re-run against the same file, or start over with a different one.
